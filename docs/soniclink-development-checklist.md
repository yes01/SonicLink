...
- [ ] 增加 APK Agent 设备字段：
  - [ ] `connection_mode`：`adb` 或 `agent`。
  - [ ] `agent_device_id`。
  - [ ] `agent_status`。
  - [ ] `agent_last_seen_at`。
  - [ ] `agent_capabilities`。
- [ ] 增加 Agent WebSocket 接口：
  - [ ] 设备注册。
  - [ ] 心跳。
  - [ ] 命令响应。
  - [ ] 二进制视频帧或投屏会话通道。
- [ ] 增加用户控制通道，或复用现有远程控制 API。
- [ ] 增加设备会话路由：
  - [ ] 用户命令 -> 后端 -> 对应 APK WebSocket。
  - [ ] APK 响应 -> 后端 -> 用户会话。
- [ ] 复用或扩展现有单用户占用/抢占逻辑。
- [ ] 增加视频流转发：
  - [ ] APK H.264 -> 后端 -> 前端 WebSocket。
  - [ ] 尽量保持与现有 JMuxer 播放链路兼容。
- [ ] 增加 API 兼容层，使前端对 ADB 设备和 Agent 设备使用统一远控动作。
- [ ] APK 断开后，服务端清理在线状态和投屏会话。
- [ ] 增加日志和指标：
  - [ ] 命令延迟。
  - [ ] 投屏 FPS。
  - [ ] 丢帧数。
...
- [ ] 显示连接方式标签：
  ...
  - [ ] SonicLink Agent。
- [ ] 尽量复用现有远程设备页面。
- [ ] Agent 设备使用 Agent 投屏 WebSocket，而不是 scrcpy WebSocket。
- [ ] 坐标映射继续复用现有逻辑，但屏幕尺寸以 Agent 上报为准。
- [ ] 根据 `connection_mode` 路由点击、滑动、按键、文本命令。
- [ ] 增加安装和配置引导：
  - [ ] 安装 APK。
  - [ ] 配置服务器地址。
  - [ ] 开启无障碍。
...
- [ ] 处理平台服务重启。
- [ ] 处理无障碍服务运行中被关闭。
- [ ] 处理 MediaProjection 授权被撤销。
- [ ] 处理息屏和锁屏。
- [ ] 处理投屏和控制过程中旋转屏幕。
- [ ] 处理刘海屏、状态栏、导航栏差异。
- [ ] 处理 Android 13/14 前台服务限制。
- [ ] 处理各厂商 ROM 后台限制。
- [ ] 所有阻塞状态都需要明确提示用户。
...
- [ ] 远程控制或投屏期间展示常驻通知。
- [ ] APK 内提供明确的停止控制按钮。
- [ ] 局域网首版可先使用 HTTP/WS，跨网使用前再补 TLS/WSS。
...
- [ ] 声明并启用无障碍服务。
- [ ] APK 能连接后端 WebSocket。
- [ ] 后端能下发点击、滑动、返回、Home 命令。
- [ ] 前端能基于静态或手动输入的屏幕尺寸控制一台手机。
